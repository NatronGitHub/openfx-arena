...
if [ "$OS" != "Msys" ]; then
  $MAKE MAGICK_DEBUG=1 USE_SVG=1 OPENCL=$USE_CL USE_PANGO=1 STATIC=1 FREEBSD=$USE_FREEBSD BITS=$BIT CONFIG=$TAG clean
  $MAKE MAGICK_DEBUG=1 USE_SVG=1 OPENCL=$USE_CL USE_PANGO=1 STATIC=1 FREEBSD=$USE_FREEBSD BITS=$BIT CONFIG=$TAG || exit 1
else
